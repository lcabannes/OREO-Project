...
class RewardDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.alfworld:
            c_action_mask = self.get_action_mask_for_alfworld(chosen_token["input_ids"], chosen_token, chosen)
            r_action_mask = self.get_action_mask_for_alfworld(reject_token["input_ids"], reject_token, reject)
        else:
            c_action_mask = torch.ones_like(chosen_token["input_ids"])
            r_action_mask = torch.ones_like(reject_token["input_ids"])
    ...
    def collate_fn(self, item_list):
        ...
        c_action_masks = []
        r_action_masks = []
        for chosen_id, chosen_mask, reject_id, rejects_mask, extra, c_action_mask, r_action_mask in item_list:
            chosen_ids.append(chosen_id)
            chosen_masks.append(chosen_mask)
            reject_ids.append(reject_id)
            rejects_masks.append(rejects_mask)
            extras.append(extra)
            c_action_masks.append(c_action_mask)
            r_action_masks.append(r_action_mask)
        ...
        c_action_masks = zero_pad_sequences(c_action_masks)
        r_action_masks = zero_pad_sequences(r_action_masks)
        return chosen_ids, chosen_masks, reject_ids, rejects_masks, extras, c_action_masks, r_action_masks

    def get_action_mask_for_alfworld(self, ids: torch.LongTensor, input_token, text: str):
        # TODO: this is for MiniCPM only
        ...
        while True:
            ...
            if i == -1:
                break
            j = text.find("<用户>", i)
            idx = input_token.char_to_token(i + 4)
            if j == -1:
                next_idx = None
            else:
                next_idx = input_token.char_to_token(j)
            mask[0, idx:next_idx] = 1
        return mask
