...
def get_llm_for_sequence_regression(
    ...
    *,
    ...
    zero_init_value_head: bool = False,
    device_map=None,
    **kwargs,
) -> nn.Module:
    ...
    # NOTE: For PCL training, it may be useful to initialize value function with zero
    if zero_init_value_head:
        if dschf is not None:
            logger.info("initialize value_head for ZeRO-3 reward model training.")
            with deepspeed.zero.GatheredParameters([model.value_head.weight], modifier_rank=0):
                if torch.distributed.get_rank() == 0:
                    model.value_head.weight.data.zero_()
        else:
            model.value_head.weight.data.zero_()
