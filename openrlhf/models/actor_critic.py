...
def get_llm_for_actor_critic(
    ...
    **kwargs,
    ...
    try:
        ...
        cls_class = _get_actor_critic_model(base_pretrained_class, base_class)
        ...
        base_pretrained_class = get_class_from_dynamic_module(f"{module_file}.{causal_model_name}", model_name_or_path)
        base_class = get_class_from_dynamic_module(f"{module_file}.{auto_model_name}", model_name_or_path)
        cls_class = _get_actor_critic_model(base_pretrained_class, base_class)
...
def _get_actor_critic_model(base_pretrained_model, base_llm_model):
    class LLMForSequenceRegression(base_pretrained_model):
        ...
        def forward(
            ...
            values = torch.zeros_like(last_hidden_states[..., 0])[:, :-1]
